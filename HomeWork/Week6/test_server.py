...
def run(host, port):
    ...
    try:
        ...
        print("Ошибка общения с сервером: {err.__class__}: {err}".format(err=err))
        sys.exit(1)
    ...
    try:
        ...
        print("Ошибка вызова client.put(...) {err.__class__}: {err}".format(err=err))
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('*') вернул неверный результат. "
                  "Ожидается: {expected_metrics}. Получено: {metrics}".format(expected_metrics=expected_metrics,
                                                                              metrics=metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('*') {err.__class__}: {err}".format(err=err))
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('k2') вернул неверный результат. "
                  "Ожидается: {expected_metrics}. Получено: {metrics}".format(expected_metrics=expected_metrics,
                                                                              metrics=metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('k2') {err.__class__}: {err}".format(err=err))
        sys.exit(1)

    try:
        result = client1.get("k3")
        if result != {}:
            print("Ошибка вызова метода get с ключом, который еще не был добавлен. " \
                   "Ожидается: пустой словарь. Получено: {result}".format(result=result))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова метода get с ключом, который еще не был добавлен: {err.__class__} {err}".format(err=err))
        sys.exit(1)
